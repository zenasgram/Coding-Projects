...
void extract_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
    ...
    int side = std::sqrt(in.size());
    int sidesub = std::sqrt(side); //represents the side of each subsquare
    int offsetV, offsetH;

    offsetV = (subs/sidesub)*sidesub;

    offsetH = (subs%sidesub)*sidesub;

    for (int i = offsetV; i < (offsetV + sidesub); i++){

        for (int j = offsetH; j < (offsetH + sidesub); j++){
